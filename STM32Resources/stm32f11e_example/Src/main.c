...
void blinkLightsTask(void const* argument)
{
    uint32_t prevWakeTime = osKernelSysTick(); //creates a variable that tracks time (not real world time -> that needs RTC real time clock that runs continuously)
    uint8_t counter = 0; //creates a counter, uint is unsigned int (never expect to be negative), 32 is number of bits, 

    while(true)
    {
        osDelayUntil(&prevWakeTime, 1000); //delay until the previous wake time + the offset of 250, then it sets wake time to when it wakes up

        switch (counter)
        {
            case 0:
//can just add more pins, changing LD3 number between 3-6
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD3_Pin,0);
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD6_Pin,0);
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD4_Pin,1);
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD5_Pin,1);
//this function toggles the pin, if pin on LED on
                counter = 1;
                break;
//LED goes on to off, this is an output pin.. never read value in, that is set somewhere else
//if you do HAL_GPIO_WritePIn(LD3_GPIO_Port, LD3_Pin, 0);
// the last argument zero is the state. 0 is off 1 is on;
            case 1:
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD3_Pin,1);
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD6_Pin,1);
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD4_Pin,0);
                HAL_GPIO_WritePin(LD3_GPIO_Port, LD5_Pin,0);
                counter = 2;
                break;
