...
elif [ "$(. /etc/os-release; echo $NAME)" = "Fedora" ]; then
...
if ! type "st-flash" > /dev/null; then
    ...
    if [ "$(. /etc/os-release; echo $NAME)" = "Ubuntu" ]; then
        ...
        echo "export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:/opt/stlink/build" >> ~/.bashrc
        export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/stlink/build
        echo "export PATH=\$PATH:/opt/stlink/build" >> ~/.bashrc
        export PATH=$PATH:/opt/stlink/build
        cp ~/.bashrc /root/
    fi
else
...
if ! type "CubeMX2Makefile" > /dev/null; then
    ...
    if [ "$(. /etc/os-release; echo $NAME)" = "Ubuntu" ]; then
        ...
        echo "export PATH=\$PATH:/opt/CubeMX2Makefile" >> ~/.bashrc
    fi
...
if [ ! -d "/opt/gcc4mbed" ]; then
    ...
    if [ "$(. /etc/os-release; echo $NAME)" = "Ubuntu" ]; then
        ...
        if ! grep "export PATH=\$PATH:/opt/gcc4mbed/gcc-arm-none-eabi/bin/" ~/.bashrc; then
            echo "export PATH=\$PATH:/opt/gcc4mbed/gcc-arm-none-eabi/bin/" >> ~/.bashrc
        fi
    fi
